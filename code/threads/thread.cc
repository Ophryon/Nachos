...
#include "sysdep.h"
// thread setting
static int threadId[MAX_THREAD] = {0};
...
Lock *producerLock = new Lock("producerLock");
Condition *producerCondition = new Condition("producerCondition");
Lock *consumerLock = new Lock("consumerLock");
Condition *consumerCondition = new Condition("consumerCondition");
void producer(int tid)
{
    ...
    for (int i = 0; i < 15; i++)
    {
        if (num == 30)
        {
            ...
            producerCondition->Wait(producerLock);
        ...
        buf[in] = item;
        num++;
        printf("tid = %d produce a new item, item = %d ,%d item left\n", tid, item, num);
        in = (in + 1) % 30;
        consumerLock->Acquire();
        consumerCondition->Signal(consumerLock);
...
void consumer(int tid)
{
    ...
    for (int i = 0; i < 10; i++)
    {
        ...
        item = buf[out];
        num--;
        printf("tid=%d consume one item, item = %d ‚%d item left\n", tid, item, num);
        out = (out + 1) % 30;
        producerLock->Acquire();
        producerCondition->Signal(producerLock);
        producerLock->Release();
        mutex->Release();
    }
}
...
Thread::Thread(char *threadName)
{
    ...
    {
        DEBUG('t', "The numbers of current threads extand maximum \n ");
        threadNum--;
        return;
    }
...
// lab8 for priority
Thread::Thread(char *threadName, int priority, int uid)
{
    // check thread number
    if (++threadNum > MAX_THREAD)
    {
...
static void
SimpleThread2(Thread *t)
{
    int num;

    for (num = 0; num < 5; num++)
    {
        cout << "*** thread " << t->getTid() << " priority " << t->getPriority() << " looped " << num << " times\n";
        kernel->currentThread->Yield();
    }
}
...
void selfTestForArttibute()
{
    DEBUG(dbgThread, "Entering selfTestForArttibute");
    Thread *t = new Thread("thread1", 1, 1);
    ThreadPrint(t);
}

void selfTestForConcurrency()
{
    DEBUG(dbgThread, "Entering SelfTestForConcurrency\nname-tid-uid-priority");
    for (int count = 0; count < MAX_THREAD + 5; count++)
    {
        Thread *t = new Thread("init thread", 0, 5446);
        ThreadPrint(t);
    }
}

void selfTestForPriority()
{
    DEBUG(dbgThread, "Entering selfTestForPriority\n");

    Thread *t1 = new Thread("forked thread1", 2, 5474);
    Thread *t2 = new Thread("forked thread2", 6, 5474);
    Thread *t3 = new Thread("forked thread3", 5, 5474);
    Thread *t4 = new Thread("forked thread4", 3, 5474);
    ThreadPrint(t1);
    ThreadPrint(t2);
    ThreadPrint(t3);
    ThreadPrint(t4);

    t1->Fork((VoidFunctionPtr)SimpleThread2, (void *)t1);
    t2->Fork((VoidFunctionPtr)SimpleThread2, (void *)t2);
    t3->Fork((VoidFunctionPtr)SimpleThread2, (void *)t3);
    t4->Fork((VoidFunctionPtr)SimpleThread2, (void *)t4);
}
// lab9
void selfTestForPC()
{
    ...
    tp->Fork((VoidFunctionPtr)producer, (void *)tp->getTid());
    Thread *tc = new Thread("consumer process");
    tc->Fork((VoidFunctionPtr)consumer, (void *)tc->getTid());
}
void Thread::SelfTest()
{
    // originalSelfTest();
    // selfTestForConcurrency();
    // selfTestForPriority();
    // selfTestForArttibute();
    selfTestForPC();
}
